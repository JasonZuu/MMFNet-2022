...
from utils.DL_load_data import FaceNet_clf_dataset, dl_load_data, split_test_verification
...
class Facenet_F_ProcSystem(Base_ProcSystem):
    def __init__(self, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), loss_gamma=1.5, init_lr=1e-4, decay_steps=5, decay_gamma=0.1):
        super().__init__()
        self.name = "Facenet_F"
        self.count_no_best = 0 # 用于记录不是最佳性能的轮次数
        self.best_accu = 0
        self.device = device
        self.extractor = InceptionResnetV1(pretrained='casia-webface',classify=True, num_classes=512).eval()
        self.extractor.to(device)
        self.frame = Facenet_F()
    ...
    def verify(self, Very_loader, save_path, epoch):
        ...
        print("-------------------------------------VERIFY----------------------------------------------------")
        print(f"current epoch:{epoch+1}\tleft epoch:{15-1-epoch}\tno_best_account{self.count_no_best}")
...
if __name__ == "__main__":
    for random_seed in range(10):
        ...
        params_path = f"./model_params/Facenet/Facenet_{random_seed}.params"
        X_train, X_test, y_train, y_test = dl_load_data("./dataset/structure.csv",random_seed, test_size=0.4) # 这一步已经打乱数据了
        X_test, X_very, y_test, y_very = split_test_verification(X_test, y_test)
        ProcSystem = Facenet_F_ProcSystem()
