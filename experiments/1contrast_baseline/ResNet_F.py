...
class FaceNet_F_ProcSystem(Base_ProcSystem):
    def __init__(self, name, device=DEVICE, eps=0.2, init_lr=1e-4, decay_steps=5, decay_gamma=0.1):
        super(FaceNet_F_ProcSystem, self).__init__()
        self.name = name
        self.count_no_best = 0
        self.best_accu = 0
        self.device = device
        self.extractor = torchvision.models.resnet18(pretrained=True).eval()
...
if __name__ == "__main__":
    for random_seed in range(5):
        ...
        ProcSystem = FaceNet_F_ProcSystem(name="ResNet_F_FringeLoss", eps=0.2)
