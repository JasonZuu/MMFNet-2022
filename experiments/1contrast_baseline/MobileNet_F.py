...
class MobileNet_F_ProcSystem(Base_ProcSystem):
    def __init__(self, name, device=DEVICE, eps=0.2, init_lr=1e-4, decay_steps=5, decay_gamma=0.1):
        super(MobileNet_F_ProcSystem, self).__init__()
        self.name = name
        self.count_no_best = 0
        self.best_accu = 0
        self.device = device
        self.extractor = MobileNetV3_Large(num_classes=1000)
        self.extractor.to(device)
        self.frame = MobileNet_F()
...
if __name__ == "__main__":
    for random_seed in range(5):
        ...
        ProcSystem = MobileNet_F_ProcSystem(name="MobileNetV3_F", eps=0.2)
