...
from models.fusion_models import ArcFace_F
from models.Fringe_loss import Fringe_Loss
import torch
import torch.optim as optim
from torch.utils.data import DataLoader
from utils.DL_load_data import Xy_Dataset_ArcFace
...
class ArcFace_F_first_PS(Base_ProcSystem):
    def __init__(self,
                name, 
                eps=0.2,
        ...
        self.extractor.load_state_dict(torch.load("../models/arcface/backbone.pth", map_location="cpu"))
        self.extractor.to(device)
        self.frame = ArcFace_F(out_planes=512)
        ...
        self.optimizer = optim.Adam([
            ...
            lr=init_lr)
        self.criterion = Fringe_Loss(eps=eps, device=device)
        

    def train(self, X, y, save_path, debug_interval=100):
        X_train, X_very, y_train, y_very = train_test_split(X, y, random_state=27, test_size=0.1)
        Train_dataset = Xy_Dataset_ArcFace(X_train, y_train)
        Train_loader = DataLoader(dataset=Train_dataset,
                                    batch_size=self.batch_size,
                                    shuffle=True) 
        Very_dataset = Xy_Dataset_ArcFace(X_very, y_very)
    ...
    def test(self, X_test, y_test):
        ...
        self.frame.eval()
        self.adaptive_module.eval()
        Test_dataset = Xy_Dataset_ArcFace(X_test, y_test)
        ...
        df.to_csv(f"./results/column/0/{self.name}.csv", sep=",", index=None)
        ...
        df.to_csv(f"./results/roc/0/{self.name}.csv", sep=",", index=None)


if __name__ == "__main__":
    ProcSystem = ArcFace_F_first_PS(name="MMFNet_Softmax")
    params_path = f"./model_params/{ProcSystem.name}/{ProcSystem.name}.params"
    datas = pd.read_csv("./dataset/tiktok_Features.csv", sep=',')
    ...
    ProcSystem.test(X_tiktok, y_tiktok)