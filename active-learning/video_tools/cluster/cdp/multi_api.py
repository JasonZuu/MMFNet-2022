...
from video_tools.cluster.cdp.source import cdp, graph, knn
...
class CDP(object):
    ...
    def knn(self, feat):
        neighbours = knn.knn_nmslib(feat, self.k, space=self.metric)
    ...
    def fit(self, X):
        ...
        assert len(X) > 0
        assert len(X[0].shape) == 2, "X should be in two dims"
        nc = len(X)
        for x in X:
            assert x.shape[0] == X[0].shape[0]
        num = X[0].shape[0]
        # knn
        knns = [self.knn(x) for x in X]
        # pair selection
        pairs, scores = cdp.sample(knns[0], knns[1:], accept=nc, th=self.th)
