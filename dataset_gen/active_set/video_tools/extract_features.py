...
import pandas as pd
from dataset_gen.active_set.video_tools.utils import get_image_size, get_image_acu
import cv2
...
class EF_system():
    ...
    def output(self):
        output_df = pd.DataFrame(self.value_dict, index=None)
        return output_df
    ...
    def clear(self):
        if not self.IS_CLEAR:
            ...
            self.fps = 0 # 视频的fps
            self.width = 0 # 视频输出帧的宽度 7020239285278346497
...
if __name__ == "__main__":
    VID = EF_system()
    batch_process(VID, "./dataset/active_100", "./dataset/active_100/Features.csv")